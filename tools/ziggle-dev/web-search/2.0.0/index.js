...
var index_default = createTool().id("web-search").name("Web Search").description("Search the web and Twitter/X using X AI's live search API with Grok. Uses API key from Clanker settings or environment.").category(ToolCategory.Utility).capabilities(ToolCapability.NetworkAccess).tags("web", "search", "twitter", "x", "ai", "api", "internet", "query").stringArg("query", "Search query to execute", { required: true }).stringArg("search_type", "Type of search: web, twitter, or all", {
  ...
  let apiKey;
  const stateApiKey = ((_a = context.state) == null ? void 0 : _a.get("apiKey")) || ((_b = context.state) == null ? void 0 : _b.get("clanker:apiKey"));
  if (stateApiKey && typeof stateApiKey === "string") {
    apiKey = stateApiKey;
    (_c = context.logger) == null ? void 0 : _c.debug("Using API key from context state");
  }
  if (!apiKey) {
    apiKey = process.env.X_AI_API_KEY || process.env.GROK_API_KEY || process.env.OPENAI_API_KEY;
    if (apiKey) {
      (_d = context.logger) == null ? void 0 : _d.debug("Using API key from environment variable");
    }
  }
  if (!apiKey) {
    (_e = context.logger) == null ? void 0 : _e.error("No API key found in settings or environment");
    return {
      success: false,
  ...
  (_f = context.logger) == null ? void 0 : _f.info(`Searching for: ${query}`);
  (_g = context.logger) == null ? void 0 : _g.debug(`Search type: ${search_type}, Max results: ${max_results}, Time range: ${time_range}`);
  try {
    ...
    (_h = context.logger) == null ? void 0 : _h.debug("Request body:", JSON.stringify(requestBody, null, 2));
    ...
    if (!response.ok) {
      ...
      (_i = context.logger) == null ? void 0 : _i.error(`API error: ${response.status} - ${errorText}`);
    ...
    if (!data.choices || data.choices.length === 0) {
      (_j = context.logger) == null ? void 0 : _j.error("No response from API");
      return {
        success: false,
    ...
    const sourcesUsed = ((_k = data.usage) == null ? void 0 : _k.num_sources_used) || 0;
...
*Searched ${sourcesUsed} sources*`;
    ...
    (_l = context.logger) == null ? void 0 : _l.info(`Search completed successfully using ${sourcesUsed} sources`);
    return {
      success: true,
      output: output.trim(),
      data: {
    ...
    (_m = context.logger) == null ? void 0 : _m.error(`Search failed: ${error instanceof Error ? error.message : String(error)}`);
    return {
      success: false,
