...
var index_default = createTool().id("elevenlabs_tts").name("ElevenLabs TTS").description("ElevenLabs text-to-speech with real-time streaming audio playback. Auto-plays Clanker messages as they stream, with smart defaults and persistent settings. Requires input tool v1.1.0+.").category(ToolCategory.Utility).capabilities(ToolCapability.NetworkAccess, ToolCapability.SystemExecute).tags("elevenlabs", "tts", "text-to-speech", "audio", "voice", "hook").stringArg("action", "Action to perform: enable, disable, or status", {
  required: true,
  ...
  if (settings) {
    ...
    if (settings.enabled) {
      ...
      if (hookEnabled && settings.apiKey) {
        try {
          (_d = context.logger) == null ? void 0 : _d.info("AUTO-INSTALLING HOOK because enabled=true in settings");
          await installHook(context);
          (_e = context.logger) == null ? void 0 : _e.info("ElevenLabs TTS hook auto-enabled from settings");
        } catch (error) {
          (_f = context.logger) == null ? void 0 : _f.error("Failed to auto-enable TTS hook:", error);
        }
      }
    }
  }
  (_g = context.logger) == null ? void 0 : _g.info("ELEVENLABS TTS INITIALIZATION COMPLETE");
...
async function enableHook(apiKeyInput, voiceIdInput, modelIdInput, autoPlay, context) {
  ...
  if (!finalVoiceId) {
    ...
    if (savedSettings == null ? void 0 : savedSettings.voiceId) {
      ...
      (_b = context.logger) == null ? void 0 : _b.info("Using default voice: Sarah");
    }
  }
  apiKey = finalApiKey;
  ...
  await installHook(context);
  (_c = context.logger) == null ? void 0 : _c.info("ElevenLabs TTS hook enabled");
  return {
    success: true,
...
async function streamSpeech(text, context) {
  ...
  const cleanText = text.replace(/```[\s\S]*?```/g, "").replace(/[*_~`#]/g, "").replace(/\[([^\]]+)\]\([^)]+\)/g, "$1").trim();
  if (!cleanText) {
    (_a = context.logger) == null ? void 0 : _a.debug("No speakable text after cleaning");
    return;
  }
  (_b = context.logger) == null ? void 0 : _b.info("Starting streaming TTS...");
...
async function installHook(context) {
  ...
  const hookId = context.hooks.register({
    ...
    handler: async (input, hookContext) => {
      var _a2, _b2, _c2, _d2;
      (_a2 = context.logger) == null ? void 0 : _a2.info(`TTS HOOK TRIGGERED! Message role: ${input.role}, has content: ${!!input.content}`);
      (_b2 = context.logger) == null ? void 0 : _b2.info(`Hook input:`, JSON.stringify(input));
      if (input.role !== "assistant" || !input.content) {
        (_c2 = context.logger) == null ? void 0 : _c2.info("Skipping non-assistant message or no content");
        return { continue: true };
      }
      (_d2 = context.logger) == null ? void 0 : _d2.info("PROCESSING ASSISTANT MESSAGE FOR TTS!");
...
async function handleTTSHook(input, context) {
  ...
  try {
    const settings = await loadToolSettings();
    (_b = context.logger) == null ? void 0 : _b.debug(`TTS settings - autoPlay: ${settings == null ? void 0 : settings.autoPlay}`);
    if ((settings == null ? void 0 : settings.autoPlay) !== false) {
      (_c = context.logger) == null ? void 0 : _c.info("Streaming TTS for message...");
    ...
    (_e = context.logger) == null ? void 0 : _e.error("TTS streaming or generation failed:", error);
    return {
      continue: true,
