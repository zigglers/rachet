...
var outputDir;
var settingsPath;
var index_default = createTool().id("elevenlabs_tts").name("ElevenLabs TTS").description("ElevenLabs text-to-speech integration that hooks into Clanker to playback all messages. Requires the input tool for API key prompting.").category(ToolCategory.Utility).capabilities(ToolCapability.NetworkAccess, ToolCapability.SystemExecute).tags("elevenlabs", "tts", "text-to-speech", "audio", "voice", "hook").stringArg("action", "Action to perform: enable, disable, or status", {
  required: true,
