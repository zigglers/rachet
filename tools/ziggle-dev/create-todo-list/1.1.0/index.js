...
function generateTodoSummary(todoList2) {
  if (todoList2.length === 0) {
    return "No todos";
  }
  const byStatus = {
    pending: todoList2.filter((t) => t.status === "pending"),
    in_progress: todoList2.filter((t) => t.status === "in_progress"),
    completed: todoList2.filter((t) => t.status === "completed")
  };
  const lines = [];
  ...
  lines.push("");
  lines.push(`Total: ${todoList2.length} | Pending: ${byStatus.pending.length} | In Progress: ${byStatus.in_progress.length} | Completed: ${byStatus.completed.length}`);
  return lines.join("\n");
}
var index_default = createTool().id("create_todo_list").name("Create Todo List").description("Create a new todo list for planning and tracking tasks").category(ToolCategory.Task).capabilities().tags("todo", "task", "planning", "tracking").arrayArg("todos", "Array of todo items", {
  ...
  (_b = context.logger) == null ? void 0 : _b.info(`Created todo list with ${currentTodoList.length} items`);
  return {
    success: true,
