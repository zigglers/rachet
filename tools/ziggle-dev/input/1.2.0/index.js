...
var index_default = createTool().id("input").name("Get User Input").description("Show platform-specific input dialogs to get information from the user. Supports text, password, dropdown selection, and chained questions for multiple inputs.").category(ToolCategory.Utility).capabilities(ToolCapability.UserConfirmation).tags("input", "dialog", "user", "prompt", "ask", "dropdown", "select", "choice").stringArg("prompt", "The prompt/question to show to the user (single question)", { required: false }).stringArg("default_value", "Default value to pre-fill in the input field (optional)", { required: false }).stringArg("title", "Title for the dialog window (optional)", { required: false, default: "Input Required" }).booleanArg("password", "Whether to mask the input for password entry (optional)", {
  required: false,
  default: false
  ...
  required: false,
  default: "text"
}).arrayArg("questions", "Array of question objects for chained inputs", { required: false }).examples([
  {
  ...
  var _a, _b, _c;
  const { prompt, default_value, title, password, options, type, questions } = args;
  ...
  try {
    ...
    (_b = context.logger) == null ? void 0 : _b.info(`User provided input: ${inputType === "password" ? "[HIDDEN]" : result}`);
    return {
      success: true,
      output: result,
      data: { input: result }
...
async function handleChainedQuestions(questions, context) {
  var _a, _b;
  const answers = {};
  const platform2 = os.platform();
  (_a = context.logger) == null ? void 0 : _a.info(`Processing ${questions.length} chained questions`);
  for (let i = 0; i < questions.length; i++) {
    ...
    try {
      ...
      answers[key] = result;
      (_b = context.logger) == null ? void 0 : _b.info(`Question ${i + 1}: ${inputType === "password" ? "[HIDDEN]" : result}`);
    } catch (error) {
  ...
  const formattedOutput = Object.entries(answers).map(([key, value]) => `${key}: ${value}`).join("\n");
  return {
    success: true,
...
function generateAnswerKey(prompt, index) {
  const patterns = [
  ...
  const significantWord = words.find(
    (w) => w.length > 3 && !["what", "enter", "your", "the", "please", "choose", "select"].includes(w)
  );
