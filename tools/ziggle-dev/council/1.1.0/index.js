...
var createTool = () => {
  ...
  const builder = {
    ...
    capabilities: (...caps) => {
      tool.capabilities = ["SystemExecute"];
      return builder;
    },
    tags: (...tags) => {
      tool.tags = tags;
...
var councilTool = createTool().id("council").name("Council").description("Summon a council of personalities to discuss topics with audio output").category("Utility").capabilities("SystemExecute").tags("council", "discussion", "debate", "tts", "audio").stringArg("topic", "The topic for the council to discuss", { required: true }).numberArg("members", "Number of council members (2-6)", { required: false, default: 4 }).numberArg("rounds", "Number of discussion rounds", { required: false, default: 3 }).booleanArg("voice", "Enable voice output via ElevenLabs TTS", { required: false, default: true }).examples([
  {
  ...
  async function speak(speaker, message) {
    ...
    if (enableVoice) {
      try {
        ...
        const voiceName = getVoiceMapping((member == null ? void 0 : member.voice) || "default");
